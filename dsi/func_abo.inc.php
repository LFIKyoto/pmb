...
// +-------------------------------------------------+
// $Id: func_abo.inc.php,v 1.40 2018-06-12 11:35:05 dgoron Exp $
...
function get_cb_dsi($title_form, $message, $form_action, $form_cb="") {
	...
	global $deflt2docs_location, $empr_location_id, $pmb_lecteurs_localises, $param_allloc;
...
function dsi_list_empr($form_cb="",$ban_priv_seuls=1) {
	...
	if ($nbr_lignes == 1) {
		...
		$url_base = $base_path."/dsi.php?categ=bannettes&sub=abo&suite=search&form_cb=".rawurlencode($form_cb)."&ban_priv_seuls=$ban_priv_seuls&empr_location_id=$empr_location_id" ;
...
function dsi_list_bannettes_abo($id_empr=0) {
	...
	$requete = "SELECT id_bannette FROM bannettes WHERE proprio_bannette='$id_empr' ORDER BY nom_bannette, id_bannette ";
	...
	while(($ban=pmb_mysql_fetch_object($res))) {
		...
		$bann_list .= "
			<td style='vertical-align:top' $td_javascript>
		...
		$bann_list .= "<td style='vertical-align:top' $td_javascript><ul>$equations</ul></td>";
		
		$bann_list .= "<td style='vertical-align:top'>$bann->nb_notices</td>";
		
		$td_javascript=" onmousedown=\"document.location='./dsi.php?categ=diffuser&sub=lancer';\" ";
		$bann_list .= "<td style='vertical-align:top' sorttable_customkey='".$bann->date_last_envoi."' $td_javascript>
				<strong>".htmlentities($bann->aff_date_last_envoi,ENT_QUOTES, $charset)."</strong>";
		if ($bann->alert_diff) $bann_list .= "<br /><span style='color:red'>(".htmlentities($bann->aff_date_last_remplissage,ENT_QUOTES, $charset).")</span>";
