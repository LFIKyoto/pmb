...
// +-------------------------------------------------+
// $Id: cms_module_recordslist_selector_indexint.class.php,v 1.1 2015-04-10 10:29:04 apetithomme Exp $
	...
	public function get_value(){
		...
		if(!$this->value){
			...
			if($sub_selector->get_value()){
				...
				$query = "select indexint from notices where notice_id = ".$sub_selector->get_value();
				$result = pmb_mysql_query($query);
				if(pmb_mysql_num_rows($result)){
					$row = pmb_mysql_fetch_object($result);
					$this->value['indexint'] = $row->indexint;
				}
			}
		}
		return $this->value;
