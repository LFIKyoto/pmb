...
// +-------------------------------------------------+
// $Id: remote_procs.inc.php,v 1.7 2015-04-03 11:16:24 jpermanne Exp $
	...
	case 'import_remote':
		if ($id) {
			if($do_import) {
				...
				$sql = "INSERT INTO empr_caddie_procs (type, name, requete, comment, autorisations, parameters) VALUES ('".$type."', '".$the_procedure->name."', '".pmb_mysql_escape_string($the_procedure->sql)."', '".$the_procedure->comment."', '".pmb_mysql_escape_string($autorisations)."', '".pmb_mysql_escape_string($parameters)."')";
