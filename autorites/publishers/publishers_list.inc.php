...
if($nbr_lignes) {
	...
	while(($ed=pmb_mysql_fetch_object($res))) {
		...
		$ed_list.= htmlentities($ed->ed_name,ENT_QUOTES,$charset);
		$ed_list.= "</td>
					<td valign='top' onmousedown=\"document.location='./autorites.php?categ=editeurs&sub=editeur_form&id=$ed->ed_id&user_input=".rawurlencode(stripslashes($user_input))."&nbr_lignes=$nbr_lignes&page=$page';\">";
		$affcall='';
		if ($ed->ed_ville || $ed->ed_pays) {
			if ($ed->ed_ville) {
				$affcall.=$ed->ed_ville;
				if($ed->ed_pays) $affcall.=' - ';
			}
			$affcall.=$ed->ed_pays;
		}
		...
		if($ed->ed_web) {
			$ed_list .= "<a href='$ed->ed_web' target='_new'>".htmlentities($ed->ed_web,ENT_QUOTES,$charset)."</a>";
		}else {
			$ed_list .= '&nbsp;';
		}
		$ed_list .= "</td>"; 
