...
function show_procs() {
	show_remote_procs("PNS");
	show_remote_procs("PNA");
	show_remote_procs("PES");
	show_remote_procs("PEA");
	show_remote_procs("PBS");
	show_remote_procs("PBA");
}
...
function display_remote_proc($id) {
	...
	if (!in_array($the_procedure->type, $allowed_proc_types)) {
	...
	$cart_proc_view_remote = str_replace('!!additional_information!!', htmlentities($additional_information,ENT_QUOTES, $charset), $cart_proc_view_remote);
 	$cart_proc_view_remote = str_replace('!!name!!', htmlentities($the_procedure->name,ENT_QUOTES, $charset), $cart_proc_view_remote);
 	$cart_proc_view_remote = str_replace('!!name_suppr!!', htmlentities(addslashes($the_procedure->name),ENT_QUOTES, $charset), $cart_proc_view_remote);
 	$cart_proc_view_remote = str_replace('!!ptype!!', htmlentities(($the_procedure->type == "PEMPS" ? $msg["caddie_procs_type_SELECT"] : $msg["caddie_procs_type_ACTION"]),ENT_QUOTES, $charset), $cart_proc_view_remote);
 	$cart_proc_view_remote = str_replace('!!code!!', htmlentities($the_procedure->sql,ENT_QUOTES, $charset), $cart_proc_view_remote);
 	$cart_proc_view_remote = str_replace('!!comment!!', htmlentities($the_procedure->comment,ENT_QUOTES, $charset), $cart_proc_view_remote);
...
//	highlight_string(print_r($parameters, true));
	if ($parameters) {
		$cart_proc_view_remote = str_replace('!!parameters_title!!', "<label class='etiquette' for='form_comment'>".htmlentities($msg["remote_procedures_procedure_parameters"],ENT_QUOTES, $charset)."</label>", $cart_proc_view_remote);
		...
		$cart_proc_view_remote = str_replace('!!parameters_content!!', $parameters_display, $cart_proc_view_remote);
	}
	
	$cart_proc_view_remote = str_replace('!!parameters_title!!', "", $cart_proc_view_remote);
	$cart_proc_view_remote = str_replace('!!parameters_content!!', "", $cart_proc_view_remote);
	
	print $cart_proc_view_remote;
	...
	case 'import_remote':
		if ($id) {
			if($do_import) {
				...
				$form = "<form class='form-$current_module' name='maj_proc' method='post' action='catalog.php?categ=caddie&sub=gestion&quoi=remote_procs&action=import_remote&id=".$id."&do_import=1'>"; 
				...
				$form .= '</div>';
				$form .= "<input type='button' class='bouton' value='$msg[76]' onClick='document.location=\"./catalog.php?categ=caddie&sub=gestion&quoi=remote_procs\"' />&nbsp;";
...
//			pmb_mysql_query($sql, $dbh) or die(pmb_mysql_error());
			...
			if (preg_match_all("|!!(.*)!!|U",$hp->proc->requete,$query_parameters))
				$hp->gen_form("catalog.php?categ=caddie&sub=gestion&quoi=remote_procs&action=final_remote&id=$id");
			else echo "<script>document.location='catalog.php?categ=caddie&sub=gestion&quoi=remote_procs&action=final_remote&id=".$id."'</script>";
		}
		else echo "<script>document.location='catalog.php?categ=caddie&sub=gestion&quoi=remote_procs&action=final_remote&id=".$id."'</script>";
