			...
			case "TABLEAU":
				...
				for($i=0; $i < $nbr_lignes; $i++) {
					...
					for($iaut=0; $iaut < $max_aut; $iaut++) {
						...
						if ($aut[4] == "70") $lib_type_aut = $msg[203];
						else if ($aut[4] == "71") $lib_type_aut = $msg[204];
						else if ($aut[4] == "72") $lib_type_aut = $msg["congres_libelle"];
						else $lib_type_aut=$aut[4];
						$worksheet->write_string($debligne_excel,($iaut*6+5+$nbr_champs),$lib_type_aut);
						$lib_resp_type="";
						if ($aut[0]) {
							if ($aut[5] == 0) $lib_resp_type=$msg["export_main_author"];
							else if ($aut[5] == 1) $lib_resp_type=$msg["export_other_author"];
							else if ($aut[5] == 2) $lib_resp_type=$msg["export_secondary_author"];
						}
