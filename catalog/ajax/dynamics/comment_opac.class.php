...
class comment_opac{
	...
	public function make_display(){
		...
		switch($this->champ_entree){			
			case 'text':
				$display = "<form method='post' name='edition'><textarea  id='save_".$this->id_element."' name='save_".$this->id_element."' cols='50' rows='6' wrap='virtual'>".htmlentities($act->rel_comment_opac,ENT_QUOTES,$charset)."</textarea> $submit</form>
				<script type='text/javascript' >document.forms['edition'].elements['save_".$this->id_element."'].focus();</script>";
				break;
			default:
				$display = "<label id='".$this->id_element."' />".htmlentities($act->rel_comment_opac,ENT_QUOTES,$charset)."</label>";
				break;
		}
