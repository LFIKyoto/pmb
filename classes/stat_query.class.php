...
class stat_query {
	...
	public function run_form($id, $dbh) {
		...
		if (preg_match_all("|!!(.*)!!|U",$hp->proc->requete,$query_parameters))
			$hp->gen_form("admin.php?categ=opac&sub=stat&section=view_list&act=final&id=$id".($force_exec ? "&force_exec=$force_exec" : ""));
		else echo "<script>document.location='admin.php?categ=opac&sub=stat&section=view_list&act=final&id=$id".($force_exec ? "&force_exec=$force_exec" : "")."'</script>";
	}
