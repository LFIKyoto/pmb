...
// +-------------------------------------------------+
// $Id: list_readers_ui.class.php,v 1.1 2018-12-27 10:05:22 dgoron Exp $
...
class list_readers_ui extends list_ui {
	...
	public function init_filters($filters=array()) {
		...
		if(get_called_class() == 'list_readers_bannette_ui') {
	...
	public function get_search_filters_form() {
		...
		global $list_readers_ui_search_filters_form_tpl;
	
		$search_filters_form = $list_readers_ui_search_filters_form_tpl;
		if ($pmb_lecteurs_localises) {
			$search_filters_form = str_replace('!!locations!!', docs_location::gen_combo_box_empr($this->filters['empr_location_id']), $search_filters_form);
		} else {
			$search_filters_form = str_replace('!!locations!!', '', $search_filters_form);
		}
		$search_filters_form = str_replace('!!status!!', gen_liste("select idstatut, statut_libelle from empr_statut","idstatut","statut_libelle","empr_statut_edit","",$this->filters['empr_statut_edit'],-1,"",0,$msg["all_statuts_empr"]), $search_filters_form);
		$search_filters_form = str_replace('!!categories!!', emprunteur::gen_combo_box_categ($this->filters['empr_categ_filter']), $search_filters_form);
		$search_filters_form = str_replace('!!codestat!!', emprunteur::gen_combo_box_codestat($this->filters['empr_codestat_filter']), $search_filters_form);
	...
	protected function get_cell_content($object, $property) {
		...
		switch($property) {
			...
			case 'categ_change':
				...
				$content .= "<select id='".$this->objects_type."_categ_change_".$object->id."' name='".$this->objects_type."_categ_change[".$object->id."]' class='saisie-20em'>";
