...
// +-------------------------------------------------+
// $Id: index_concept.class.php,v 1.10 2015-06-25 12:04:37 arenou Exp $
...
class index_concept {
	...
	public function get_form($caller) {
		global $index_concept_form, $index_concept_script, $index_concept_add_button_form, $index_concept_text_form;
	 ...
	 */
	private function get_from_form() {
		...
		if (!count($this->concepts) && $this->object_id) {
			...
			if ($result && pmb_mysql_num_rows($result)) {
				while ($row = pmb_mysql_fetch_object($result)){
					$this->concepts[$row->order_concept] = new concept($row->num_concept);
				}
			}
		}
