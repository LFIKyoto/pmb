	...
	public function get_table($form_link=""){
		...
		for($i=0 ; $i<count($types) ; $i++){
			...
			<tr class='".($i%2 ? "odd":"even")."' onmouseover=\"this.className='surbrillance'\" onmouseout=\"this.className='$pair_impair'\">
				<td onclick='document.location=\"".$form_link."&id=".$types[$i]['id']."\"'style='cursor:pointer' >".htmlentities($types[$i]['label'],ENT_QUOTES,$charset)."</td>
				<td onclick='document.location=\"".$form_link."&id=".$types[$i]['id']."\"'style='cursor:pointer' >".htmlentities($types[$i]['comment'],ENT_QUOTES,$charset)."</td>
