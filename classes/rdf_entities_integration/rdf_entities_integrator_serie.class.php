...
// +-------------------------------------------------+
// $Id: rdf_entities_integrator_serie.class.php,v 1.5 2018-06-26 14:48:14 apetithomme Exp $
...
class rdf_entities_integrator_serie extends rdf_entities_integrator_authority {
	...
	protected function init_map_fields() {
		$this->map_fields = array_merge(parent::init_map_fields(), array(
				'http://www.pmbservices.fr/ontology#label' => 'serie_name'
		));
	...
	protected function post_create($uri) {
		...
		if ($this->integration_type && $this->entity_id) {
			...
			$query.= 'values ("'.AUDIT_SERIE.'", "'.$this->entity_id.'", "'.$this->contributor_id.'", "'.$this->integration_type.'", "'.addslashes(json_encode(array("uri" => $uri))).'", "'.$this->contributor_type.'")';
			pmb_mysql_query($query);
			// Indexation
			serie::update_index($this->entity_id);
		}
	}
}