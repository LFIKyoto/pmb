...
// +-------------------------------------------------+
// $Id: rdf_entities_integrator_indexint.class.php,v 1.5 2018-06-26 14:48:14 apetithomme Exp $
...
class rdf_entities_integrator_indexint extends rdf_entities_integrator_authority {
	...
	protected function post_create($uri) {
		...
		if ($this->integration_type && $this->entity_id) {
			...
			$query.= 'values ("'.AUDIT_INDEXINT.'", "'.$this->entity_id.'", "'.$this->contributor_id.'", "'.$this->integration_type.'", "'.addslashes(json_encode(array("uri" => $uri))).'", "'.$this->contributor_type.'")';
