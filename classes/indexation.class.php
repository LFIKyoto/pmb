...
// +-------------------------------------------------+
// $Id: indexation.class.php,v 1.54.4.5 2019-11-28 11:06:52 dgoron Exp $
...
class indexation {
	...
	public function maj($object_id,$datatype='all'){	
		global $dbh, $lang, $include_path, $charset, $base_path;
					...
					if($datatype == "all" || (isset($this->datatypes[$datatype]) && in_array($k,$this->datatypes[$datatype]))){
						...
						if(pmb_mysql_num_rows($res)) {
							switch ($infos["class_name"]){
								case 'author':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_AUTHORS, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'editeur':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_PUBLISHERS, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'indexint':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_INDEXINT, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'collection':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_COLLECTIONS, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'subcollection':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_SUB_COLLECTIONS, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'serie':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_SERIES, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'categories':
									while($row= pmb_mysql_fetch_object($res)){
									    $aut= authorities_collection::get_authority(AUT_TABLE_CATEGORIES, $row->id_aut_for_isbd,array('lang'=>$lang,'for_indexation' => true));
										$isbd_s[]=$aut->libelle_categorie;
									}
									break;
								case 'titre_uniforme':
									while($row= pmb_mysql_fetch_object($res)){
										$aut= authorities_collection::get_authority(AUT_TABLE_TITRES_UNIFORMES, $row->id_aut_for_isbd);
										$isbd_s[]=$aut->get_isbd();
									}
									break;
								case 'authperso':
