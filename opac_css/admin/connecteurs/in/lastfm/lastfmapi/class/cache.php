...
class lastfmApiCache {
	...
	public function get($unique_vars) {
		global $charset;
		if ( $this->enabled == true ) {
			$query = "SELECT expires, body FROM cache WHERE unique_vars='".htmlentities(serialize($unique_vars), ENT_QUOTES, $charset)."' LIMIT 1";
	...
	public function set($unique_vars,  $body) {
		...
		if ( $this->enabled == true ) {
			...
			$query = "INSERT INTO cache (unique_vars, expires, body) VALUES ('".htmlentities(serialize($unique_vars), ENT_QUOTES, $charset)."', '".$expire."', \"".htmlentities(serialize($body), ENT_QUOTES, $charset)."\")";
			if ( $this->db->query($query) ) {
				return true;
			}
			else {
	...
	private function del($unique_vars) {
		global $charset;
		$query = "DELETE FROM cache WHERE unique_vars='".htmlentities(serialize($unique_vars), ENT_QUOTES, $charset)."'";
		if ( $this->db->query($query) ) {
			return true;
		}
		else {
