...
// +-------------------------------------------------+
// $Id: coll_see.inc.php,v 1.61.2.2 2015-10-16 12:25:16 jpermanne Exp $
...
if($id) {
	...
	//collection associée
	$query = "select sub_coll_id, sub_coll_name from sub_collections where sub_coll_parent=".$id;
	$result = pmb_mysql_query($query,$dbh);
	if(pmb_mysql_num_rows($result)){
		$context['authority']["subcollections"] = array();
		while($row = pmb_mysql_fetch_object($result)){
			$context['authority']["subcollections"][]= array(
				'id' => $row->sub_coll_id,
				'name' => $row->sub_coll_name		
			);
		}
	}
	...
	$recordslist = "<h3><span class=\"aut_details_liste_titre\">$msg[available_docs_in_coll]</span></h3>\n";
