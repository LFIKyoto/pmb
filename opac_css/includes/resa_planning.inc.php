...
// +-------------------------------------------------+
// $Id: resa_planning.inc.php,v 1.10 2015-06-24 15:36:20 dbellamy Exp $
...
if ($opac_resa && $opac_resa_planning) { //resa autorisées dans l'opac et mode planning
	...
	if ($popup_resa && ($id_notice || $id_bulletin)) { // est-on appelé par le popup ? Si oui, pose réservation
		...
		print "<h3><span>".$msg['empr_bt_show_resa_planning']."</span></h3>";
		...
		if(pmb_mysql_num_rows($r3)) {
			...
			$tableau_resa.='<tr><th>'.$msg['resa_planning_title'].'</th><th>'.$msg['resa_planning_dates'].'</th>
								<th>'.$msg['resa_planning_qty'].'</th><th>'.$msg['resa_planning_loc_retrait'].'</th><th></th></tr>';
			while ($resa = pmb_mysql_fetch_array($r3)) {
				...
				$link_del="<a href='empr.php?lvl=resa_planning&delete=1&id_resa=".$id_resa."'>".$msg['resa_effacer_resa']."</a>";
		...
		print "<br /><small><br />".$msg["empr_resa_how_to"]." <br />
