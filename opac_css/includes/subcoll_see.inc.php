...
// +-------------------------------------------------+
// $Id: subcoll_see.inc.php,v 1.59.2.2 2015-10-16 12:25:16 jpermanne Exp $
...
if($id) {
	...
	$recordslist = "<div id='aut_details_liste'>\n";
	// affichage des notices associées
	
	$recordslist.= "<h3><span class=\"aut_details_liste_titre\">$msg[available_docs_in_subcoll]</span></h3>\n";
	...
	if(!$nbr_lignes) {
		...
		//$requete .= " where (subcoll_id='$i'd) and (statut=id_notice_statut and ((notice_visible_opac=1 and notice_visible_opac_abon=0)".($_SESSION["user_code"]?" or (notice_visible_opac_abon=1 and notice_visible_opac=1)":"")."))";
		$requete = "SELECT COUNT(1) FROM notices $acces_j $statut_j ";
		$requete.= " where subcoll_id='$id' $statut_r ";
		...
		$clause = "where subcoll_id='$id' $statut_r";
		$req_noti = "select distinct notices.typdoc, count(explnum_id) as nbexplnum FROM notices left join explnum on explnum_mimetype in ($opac_photo_filtre_mimetype) and explnum_notice=notice_id  $acces_j $statut_j $clause";
		$req_bull = "select distinct notices.typdoc, count(explnum_id) as nbexplnum FROM notices left join bulletins on bulletins.num_notice = notice_id and bulletins.num_notice != 0 left join explnum on explnum_mimetype in ($opac_photo_filtre_mimetype) and explnum_bulletin=bulletin_id and explnum_bulletin != 0 $acces_j $statut_j $clause";
		$requete ="select distinct typdoc, sum(nbexplnum) as nbexplnum from ($req_bull union $req_noti) as uni group by typdoc";
		$res = pmb_mysql_query($requete, $dbh);
	...
	if($nbr_lignes) {
		...
		if($opac_visionneuse_allow && $nbexplnum_to_photo){
			...
			$sendToVisionneuseByGet = str_replace("!!mode!!","subcoll_see",$sendToVisionneuseByGet);
...
// 		$recordslist.= "</div><!-- fermeture #aut_details_liste -->\n";
		$recordslist.= "<div id='navbar'><hr /><center>".printnavbar($page, $nbepages, "./index.php?lvl=subcoll_see&id=$id&page=!!page!!&nbr_lignes=$nbr_lignes&l_typdoc=".rawurlencode($l_typdoc))."</center></div>\n";
	...
	$authority = new authority("subcollection", $id);
