...
function search_other_function_clause() {
	...
	$r = "";
	if ($cnl_bibli) {
		$r="select distinct notice_id from notices where notice_id in (select expl_notice from exemplaires where expl_section='$cnl_bibli' UNION select  bulletin_notice from bulletins join exemplaires on expl_bulletin=bulletin_id  where expl_section='$cnl_bibli' )";
	}
	return $r;
}
