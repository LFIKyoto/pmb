...
// +-------------------------------------------------+
// $Id: titre_uniforme_see.inc.php,v 1.25.2.2 2015-10-16 12:25:16 jpermanne Exp $
...
if ($id) {
	...
	$recordslist = "<h3><span class=\"aut_details_liste_titre\">".$msg["doc_titre_uniforme_title"]."</span></h3>";
	...
	if ($nbr_lignes) {
		...
		if ($nbr_lignes <= $opac_nb_max_tri) {
			...
			if ($_SESSION ["last_sortnotices"] != "") {
				$recordslist .= " " . $msg ['tri_par'] . " " . $sort->descriptionTriParId ( $_SESSION ["last_sortnotices"] ) . "<span class=\"espaceResultSearch\">&nbsp;</span>";
			}
		} else
