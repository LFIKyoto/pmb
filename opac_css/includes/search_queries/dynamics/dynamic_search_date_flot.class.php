...
// +-------------------------------------------------+
// $Id: dynamic_search_date_flot.class.php,v 1.3 2018-04-18 09:35:24 ngantier Exp $
		...
		switch (${$op}) {
			...
			case 'BETWEEN': 
				...
				*/
				...
				break;				
			case 'ISEMPTY':
				$main = "select distinct " . $this->search->keyName . " from " . $this->search->tableName . $this->get_join_query() . "  
						left join " . $this->prefix . "_custom_values on " . $this->prefix . "_custom_origine=" . $this->search->keyName . "
								and " . $this->prefix . "_custom_champ = ". $this->id . "						
						where (". $this->prefix . "_custom_small_text is NULL or  ". $this->prefix . "_custom_small_text = '') ";				
				break;
			case 'ISNOTEMPTY':
				$main = "select distinct " . $this->prefix . "_custom_origine as notice_id  from " . $this->prefix . "_custom_values "  . $this->get_join_query() . "
						where " . $this->prefix . "_custom_champ = ". $this->id . " and ". $this->prefix . "_custom_small_text != '' ";
				break;				
			default:
				break;
		}
	...
	protected function get_join_query() {
		...
		switch ($this->search->tableName) {
			case 'authorities':
				switch($this->prefix) {
					case 'author':
						$join_query .= " join authors on authors.author_id=authorities.num_object and authorities.type_object = 1 join author_custom_dates on author_custom_origine=author_id ";
						break;
					case 'categ':
						$join_query .= " join noeuds on noeuds.id_noeud=authorities.num_object and authorities.type_object = 2 join categ_custom_dates on categ_custom_origine=id_noeud ";
						break;
					case 'publisher':
						$join_query .= " join publishers on publishers.ed_id=authorities.num_object and authorities.type_object = 3 join publisher_custom_dates on publisher_custom_origine=ed_id ";
						break;
					case 'collection':
						$join_query .= " join collections on collections.collection_id=authorities.num_object and authorities.type_object = 4 join collection_custom_dates on collection_custom_origine=collection_id ";
						break;
					case 'subcollection':
						$join_query .= " join sub_collections on sub_collections.sub_coll_id=authorities.num_object and authorities.type_object = 5 join subcollection_custom_dates on subcollection_custom_origine=sub_coll_id ";
						break;
					case 'serie':
						$join_query .= " join series on series.serie_id=authorities.num_object and authorities.type_object = 6 join serie_custom_dates on serie_custom_origine=serie_id ";
						break;
					case 'tu':
						$join_query .= " join titres_uniformes on titres_uniformes.tu_id=authorities.num_object and authorities.type_object = 7 join tu_custom_dates on tu_custom_origine=tu_id ";
						break;
					case 'indexint':
						$join_query .= " join indexint on indexint.indexint_id=authorities.num_object and authorities.type_object = 8 join indexint_custom_dates on indexint_custom_origine=indexint_id ";
						break;
					case 'authperso':
						$join_query .= " join authperso_authorities on id_authperso_authority=authorities.num_object and authorities.type_object = 9 join authperso_custom_dates on authperso_custom_origine=id_authperso_authority";
						break;
				}
				break;
			case 'notices':
				switch($this->prefix) {
					case 'author':
						$join_query .= " join responsability on responsability.responsability_notice=notices.notice_id join author_custom_dates on author_custom_origine=responsability.responsability_author ";
						break;
					case 'categ':
					...
					case 'serie':
						...
						break;
					case 'tu':
						$join_query .= " join notices_titres_uniformes on notices_titres_uniformes.ntu_num_notice = notices.notice_id join titres_uniformes on titres_uniformes.tu_id = notices_titres_uniformes.ntu_num_tu join tu_custom_dates on tu_custom_origine=tu_id ";
						break;
					case 'indexint':
