...
// +-------------------------------------------------+
// $Id: subcollection.inc.php,v 1.25 2015-04-16 16:09:56 arenou Exp $
...
if(!$opac_allow_affiliate_search || ($opac_allow_affiliate_search && $tab == "catalog")){
	...
	$found = pmb_mysql_query("select sub_coll_id, ".$pert.",sub_coll_name from sub_collections $clause group by sub_coll_id $tri $limiter", $dbh);

	while($mesSousCollections = pmb_mysql_fetch_object($found)) {
		print pmb_bidi("<li class='categ_colonne'><font class='notice_fort'><a href='index.php?lvl=subcoll_see&id=".$mesSousCollections->sub_coll_id."&from=search'>".$mesSousCollections->sub_coll_name."</a></font></li>\n");
	}
	print "</ul>";
	print "
	</div></div>";
