...
class tu_notice {
	...
	public function get_print_type($link='') {
		...
		if($this->ntu_data && is_array($this->ntu_data)){
			foreach ($this->ntu_data as $tu) {
				if($link){
					$link_print="<a href='".$link.$tu->num_tu."' class='lien_gestion'>".$tu->tu->display."</a>";
				}else {
					$link_print=$tu->num_tu;
				}
				$biblio_fields=array();
				...
				$display.=" ".$link_print.$biblio_print."<br />";
	...
	public static function gen_input_selection($label,$form_name,$item,$values,$what_sel,$class='saisie-80em' ) {
	    ...
	    function add_".$item."() {
			...
			img_".$item.".setAttribute('src','./getgif.php?nomgif=plus');  
		...
		$script_js_option="
	        ...
	        op_!!num!!.setAttribute('name','!!name!!_!!num!!'+suffixe);
	        op_!!num!!.setAttribute('id','!!name!!_!!num!!'+suffixe);
	        op_!!num!!.setAttribute('type','text');
	        op_!!num!!.className='!!class!!';
	        op_!!num!!.setAttribute('value','');
	        div_!!num!!.appendChild(op_!!num!!);
	        
			//template.appendChild(div_label_!!num!!);
	    	//template.appendChild(div_!!num!!);
		...
		<div style='display: block;' id='tu!!num!!Parent' class='parent'>
			<img src='./getgif.php?nomgif=plus' class='img_plus' name='imEx' id='tu!!num!!Img' title='".$msg["writing_zone"]."' alt='".$msg['expand']."' onclick=\"expandBase('tu!!num!!', true); return false;\" style='border:0px'>
		...
		$aff_option="
		<div class='row'>
			<label for='!!name!!_!!num!!' class='etiquette'>!!label!!</label></div>
		<div class='row'>
			<input type='text' class='!!class!!' id='!!name!!_!!num!!' name='!!name!!_!!num!!' value=\"!!value!!\" />
		</div>";	
