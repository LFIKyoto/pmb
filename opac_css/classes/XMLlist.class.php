...
// +-------------------------------------------------+
// $Id: XMLlist.class.php,v 1.26 2015-04-07 15:21:12 vtouchard Exp $
...
class XMLlist {
	...
	var $flag_fav;
	var $s;
	var $no_cache; // rechargement, même si cache
	...
	// constructeur
	function XMLlist($fichier, $s=1, $no_cache=0) {
 ...
 // Modif Armelle Nedelec recherche de l'encodage du fichier xml et transformation en charset'
 	function analyser() 
 	{
		...
		$dejaParse = false;
		if (!$this->no_cache && file_exists($tempFile) ) {
		...
		if ($dejaParse) {
			...
			fwrite($tmp,serialize(array($this->table,$this->table_js)));
			fclose($tmp);
		}
	}
}
