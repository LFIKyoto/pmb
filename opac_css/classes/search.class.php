...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.148.2.2 2015-12-04 16:15:55 jpermanne Exp $
...
class search {
	...
	function make_stripslashes_test_array($value){
		...
		if ($s[0]=="f") {
			...
			for ($j=0; $j<count($ff["VAR"]); $j++) {
				if ($ff["VAR"][$j]["TYPE"]=="input") {
					...
					if ($visibility) {
						$r.="&nbsp;";
						if ($ff["VAR"][$j]["OPTIONS"]["INPUT"][0]["CLASS"]) {
							$r.="<span class='".$ff["VAR"][$j]["OPTIONS"]["INPUT"][0]["CLASS"]."'>";
						}
						if ($ff["VAR"][$j]["SPAN"]) {
							$r.="<span class='".$ff["VAR"][$j]["SPAN"]."'>".$ff["VAR"][$j]["COMMENT"]."</span>";
						} else {
							$r.=$ff["VAR"][$j]["COMMENT"];
						}
	...
	function make_search($prefixe="") {
		...
		for ($i=0; $i<count($search); $i++) {
			...
			pmb_mysql_query($requete,$dbh);
			$requete="alter table ".$prefixe."t_".$i."_".$search[$i]." add pert decimal(58,1) default 1";
			@pmb_mysql_query($requete,$dbh);
			...
			switch ($$inter) {
				case "and":
    				...
    				$requete.=" from $last_table,$table where ".$table.".notice_id=".$last_table.".notice_id group by notice_id";
    				@pmb_mysql_query($requete,$dbh);
    				break;
				case "or":
					...
					if (!@pmb_mysql_result(pmb_mysql_query($requete_c),0,0)) {
	    				...
	    				@pmb_mysql_query($requete,$dbh);
	    				$requete="alter table ".$prefixe."t".$i." add pert decimal(58,1) default 1";
	    				@pmb_mysql_query($requete,$dbh);
	
	    				$requete="insert into ".$prefixe."t".$i." (notice_id,idiot,pert) select distinct ".$last_table.".notice_id,".$last_table.".idiot, ".$last_table.".pert AS pert from ".$last_table." left join ".$table." on ".$last_table.".notice_id=".$table.".notice_id where ".$table.".notice_id is null";
	    				@pmb_mysql_query($requete,$dbh);
					}
    				break;
				case "ex":
			...
			if (array_key_exists($s[0],$this->pp)) {
				...
				switch ($this->fixedfields[$s[1]]["INPUT_TYPE"]) {
					...
					case "query_list":
						...
						if ($this->fixedfields[$s[1]]["INPUT_FILTERING"] == "yes") {
							$this->access_rights();
							$requete = str_replace("!!acces_j!!", $this->tableau_access_rights["acces_j"], $requete);
							$requete = str_replace("!!statut_j!!", $this->tableau_access_rights["statut_j"], $requete);
							$requete = str_replace("!!statut_r!!", $this->tableau_access_rights["statut_r"], $requete);
	...
	function show_results_unimarc($url,$url_to_search_form,$hidden_form=true,$search_target="") {
		...
		if ($opac_show_suggest) {
			...
			$bt_sugg.= " title='".$msg["empr_bt_make_sugg"]."' >".$msg[empr_bt_make_sugg]."</a>";
			print $bt_sugg;
		}
		flush();
	...
	function show_form($url,$result_url,$result_target='') {
		...
		for ($i=0; $i<count($search); $i++) {
			if ((string)$i!=$delete_field) {
				...
				if(!$limitsearch){
					$r.="<td ".(in_array("6",$notdisplaycol)?"style='display:none;'":"")."><span class='search_cancel'>".(!$delnotallowed?"<input type='button' class='bouton' value='".$msg["raz"]."' onClick=\"this.form.delete_field.value='".$n."'; this.form.action='$url'; this.form.target=''; this.form.submit();\">":"&nbsp;")."</span>";//Colonne 6
					$r.="</td>";
				}
				$r.="</tr>\n";
	...
	function parse_search_file($fichier_xml) {
		...
		if ($fichier_xml=="") {
			$fichier_xml = "search_fields";
			}
		$filepath = $include_path."/search_queries/".$fichier_xml."_subst.xml";
		if (!file_exists($filepath)) {
			$filepath = $include_path."/search_queries/".$fichier_xml.".xml";
		}
		...
		if ($dejaParse) {
			...
			fclose($tmp);
			if(count($cache) == 8){
				$this->memory_engine_allowed = $cache[0];
				$this->operators = $cache[1];
				$this->op_empty = $cache[2];
				$this->fixedfields = $cache[3];
				$this->dynamics_not_visible = $cache[4];
				$this->dynamicfields = $cache[5];
				$this->specialfields = $cache[6];
				$this->tableau_speciaux = $cache[7];
			}else{
				//SOUCIS de cache...
				unlink($tempFile);
				$this->parse_search_file($fichier_xml);
		...
		for ($i=0; $i<count($param["FIXEDFIELDS"][0]["FIELD"]); $i++) {
			...
			$t["INPUT_FILTERING"]=$ff["INPUT"][0]["FILTERING"];
		...
		if(!$param["DYNAMICFIELDS"][0]["FIELDTYPE"]){//Pour le cas de fichiers subst basés sur l'ancienne version
		...
		for ($i=0; $i<count($param["SPECIALFIELDS"][0]["FIELD"]); $i++) {
			...
			$t["OPACVISIBILITY"]=($sf["OPACVISIBILITY"]=="no"?false:true);
