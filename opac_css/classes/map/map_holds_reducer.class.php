...
class map_holds_reducer {
	...
	public function init(){
		 ...
		 */
		$this->map_distance = (sqrt(pow(($map_coords[0]->get_decimal_long() - $map_coords[3]->get_decimal_long()),2) + pow(($map_coords[0]->get_decimal_lat() - $map_coords[3]->get_decimal_lat()),2)))/$opac_map_hold_distance;
	}
	...
	public function get_reduction(){
		global $opac_map_hold_ratio_max, $opac_map_hold_ratio_min;

		$this->dicho($opac_map_hold_ratio_min, $opac_map_hold_ratio_max);
	...
	public function dicho($min, $max){
		global $opac_map_max_holds, $opac_map_hold_ratio_min;
		$params = explode(",", $opac_map_max_holds);
		...
		$mid = (($max-$min)/2)+$min;
		if((($max-$min)/2)  <= $opac_map_hold_ratio_min){
			$this->calc_empr($mid);
