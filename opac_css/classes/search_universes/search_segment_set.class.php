...
// +-------------------------------------------------+
// $Id: search_segment_set.class.php,v 1.4 2018-05-18 07:56:32 vtouchard Exp $
...
// 	    $search->json_decode_search($this->data_set);
	    ...
	    if (isset($this->type)) {
	        switch($this->type) {
	            case TYPE_NOTICE :
	                ...
	                break;
	            default :
	                $this->search_instance = new search_authorities('search_fields_authorities');
	                break;
	        }
	        return $this->search_instance;
	    }
	...
	//Permet de savoir si nous avons des notices ou des autorités ?
	...
	public function make_search($prefix = '') {
		...
		if (empty($this->data_set)) {
			...
			$this->data_set = combine_search::simple_search_to_mc(stripslashes('*'), true, $this->type);
