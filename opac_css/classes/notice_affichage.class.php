...
// +-------------------------------------------------+
// $Id: notice_affichage.class.php,v 1.415.2.13 2015-11-14 12:45:36 Alexandre Exp $
...
class notice_affichage {
	...
	function avis_detail () {
		...
		global $opac_avis_allow,$opac_avis_show_writer;
		...
		$sql_avis = "select note,sujet,commentaire,DATE_FORMAT(dateajout,'".$msg['format_date']."') as ladate,empr_login,empr_nom, empr_prenom
				from avis
		...
		if (pmb_mysql_num_rows($r_avis)) {
			...
			while (($data=pmb_mysql_fetch_object($r_avis))) {
				...
				if($opac_avis_note_display_mode){
					...
					$ret .= "<td class='avis_detail_note_".$data->note."'  ><span class='avis_note'>".$aff."</span></td>";
				}
				$ret .= "
					<td class='avis_detail_commentaire_".$data->note."'>
						<span class='avis_entete'>
							<b>".htmlentities($data->sujet,ENT_QUOTES,$charset)."</b>, ".$data->ladate;
				if ($opac_avis_show_writer==1 && $data->empr_nom){
					$ret .= " ".$msg['avis_de']." $data->empr_prenom $data->empr_nom ";
				}
				if ($opac_avis_show_writer==2 && $data->empr_login){
					$ret .= " ".$msg['avis_de']." $data->empr_login ";
				}
				$ret.="	</span>
						<br />
...
//			}
	...
	function gen_coins_span(){
		...
		switch ($this->notice->niveau_biblio){
			...
			case 'a': // article
				...
				if($this->parent_numero) $coins_span.="&amp;rft.issue=".rawurlencode($f?$f($this->parent_numero):$this->parent_numero);
				if($this->parent_date){
					$coins_span.="&amp;rft.date=".rawurlencode($f?$f($this->parent_date):$this->parent_date);
				}elseif($this->parent_aff_date_date){
					$coins_span.="&amp;rft.date=".rawurlencode($f?$f($this->parent_aff_date_date):$this->parent_aff_date_date);
				}
	...
	function get_serialcirc_form_actions(){
		...
		if($_SESSION['id_empr_session'] && $opac_serialcirc_active && $this->notice->opac_serialcirc_demande){
			if($this->notice->niveau_biblio == "s"){
