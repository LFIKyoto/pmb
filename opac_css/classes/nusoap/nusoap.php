...
/*
$Id: nusoap.php,v 1.13 2017-10-27 14:08:51 ngantier Exp $
...
* @version  $Id: nusoap.php,v 1.13 2017-10-27 14:08:51 ngantier Exp $
* @access   public
*/
class nusoap_base {
	/**
	...
	*/
	function invoke_method() {
		...
		if ($class == '') {
			...
			if (in_array($method_to_compare, get_class_methods($class))===false) {
			                ...
			                for ($ii = 0; $ii < count($list2); $ii++) {
			                	if (! $list2[$ii]['loaded']) {
									...
									if ($url != '') {
										...
										if (in_array($url, $imported_urls)===false) {
						                	$this->parseWSDL($url);
					                		$imported++;
					                		$imported_urls[] = $url;
					                	}
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					if(isset($opParts['documentation']) && $opParts['documentation'] != '') {
						$portType_xml .= '<documentation>' . htmlspecialchars($opParts['documentation'],ENT_QUOTES,$charset) . '</documentation>';
					}
		...
		if ($phpType == 'struct') {
			...
			if (is_array($value) && sizeof($value) >= 1) {
				...
				foreach($value as $k => $v) {
					...
					if (in_array($typeDef['arrayType'],$this->typemap['http://www.w3.org/2001/XMLSchema'])===false) {
...
//				}
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		// set name
		$this->message[$pos]['name'] = htmlspecialchars($name,ENT_QUOTES,$charset);
		// set attrs
