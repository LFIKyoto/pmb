...
function getData() {
	...
	if ($nb_notices=pmb_mysql_num_rows($resultat)) {
		while($r=pmb_mysql_fetch_object($resultat)){
			...
			$letter=pmb_substr(pmb_strtolower(trim($r->index_sew)),0,1);
			...
			if(!$ongletInc || $qtOngletCours==$this->max_per_onglet || (isset($lastLetter) && $letter!=$lastLetter && (is_numeric($lastLetter) || $lastLetter==" "))){
				$ongletInc++;
				$qtOngletCours=0;
				$this->onglets_contens[$ongletInc]["first_label"]=pmb_strtoupper(trim($r->index_sew));
			...
			$qtOngletCours++;
			$this->onglets_contens[$ongletInc]["last_label"]=pmb_strtoupper(trim($r->index_sew));
