...
// +-------------------------------------------------+
// $Id: notice_tpl.inc.php,v 1.84.2.10 2015-11-25 14:35:06 dbellamy Exp $
...
function aff_categories_with_tpl($param) {
	...
	//Descripteurs
	if ($param[0]>0) $restrict_thes=" AND num_thesaurus in (".$param[0].")";
...
function aff_gen_plus($param) {
	...
	<div id="'.$parser_environnement['id_notice'].' class="notice-parent">
		<img src="'.$pmb_opac_url.'getgif.php?nomgif=plus" class="img_plus" name="imEx" id="'.$parser_environnement['id_notice'].'Img" title="'.addslashes($msg['plus_detail']).'" border="0" onClick="expandBase("'.$parser_environnement['id_notice'].'", true); return false;" hspace="3">
...
function aff_categories($param) {
	...
	//Descripteurs
	if ($param[0]>0) $restrict_thes=" and catlg.num_thesaurus in (".$param[0].") and catdef.num_thesaurus in (".$param[0].") ";
...
function aff_header_link($param) {
	global $pmb_opac_url,$use_opac_url_base;
...
function aff_expl_num_vign_reduit($param) {
	...
	if ($result && pmb_mysql_num_rows($result)) {
		if(pmb_mysql_num_rows($result) > 1){
			...
			$img_multip_doc = $pmb_opac_url.'images/globe_orange.png';
			if(is_array($param) && (count($param) >= 5) && (trim($param[4]))) {
				$img_multip_doc=trim($param[4]);
			}

			$ret.= '<span class="expl_num_vign_reduit" ><a href="'.$pmb_opac_url.'doc_num.php?explnum_id='.$explnumrow->explnum_id.'" target="__LINK__">';
