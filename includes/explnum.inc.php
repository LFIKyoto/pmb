...
function explnum_add_from_url($f_notice_id, $f_bulletin_id, $f_nom, $f_url, $overwrite=true, $source_id=0, $filename='', $f_path='', $f_statut) {
	...
	//copie en r�pertoire temporaire
	$r=false;
	...
	if ($r) {
		...
		//si la source du connecteur est pr�cis�e, on regarde si on a pas un r�pertoire associ�
		$rep_upload=0;
		if ($source_id){
...
//			$metas = json_decode(implode("\n",$metas),true);
		...
		default :
			...
			switch ($type){
				...
				case "text" :
					//pas de m�tas pertinantes sur une fichier texte...
					break;
				default :
					if(!extension_fichier(basename($filename))){
						$new_name=basename($filename)."temp";//Pour �viter que si pas d'extension on perde le fichier
					}else{
						$new_name = str_replace(extension_fichier(basename($filename)),"pdf",basename($filename));
					}
					$new_path = dirname($filename)."/".$new_name;
					exec("curl http://localhost:8080/converter/converted/".$new_name." -F \"inputDocument=@$filename\" > ".$new_path);//Ne doit marcher que dans un cas tr�s pr�cis, pas vrai Arnaud
...
// fonction qui permet de savoir si les exemplaires num�riques pour une notice ou un bulletin donn� sont affichable � l'OPAC
