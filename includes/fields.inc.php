...
function aff_date_box($field,&$check_scripts) {
	...
	$ret="<input type='hidden' name='".$field[NAME]."' value='$val' />
				<input class='bouton' type='button' name='".$field[NAME]."_lib' value='".formatdate($val_popup)."' onClick=\"openPopUp('./select.php?what=calendrier&caller=".$form_name."&date_caller=".$val_popup."&param1=".$field[NAME]."&param2=".$field[NAME]."_lib&auto_submit=NO&date_anterieure=YES', 'date_".$field[NAME]."', 250, 300, -2, -2,'toolbar=no, dependent=yes, resizable=yes')\" />";
...
function aff_list($field,&$check_scripts) {
	...
	for ($i=0; $i<count($options[ITEMS][0][ITEM]); $i++) {
		$ret.="<option value=\"".htmlentities($options[ITEMS][0][ITEM][$i][VALUE],ENT_QUOTES,$charset)."\" ".($$param == $options[ITEMS][0][ITEM][$i][VALUE] ? "selected" : "").">".htmlentities($options[ITEMS][0][ITEM][$i][value],ENT_QUOTES,$charset)."</option>\n";
	}
	$ret.= "</select>\n";
	return $ret;
}
