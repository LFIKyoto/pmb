...
// +-------------------------------------------------+
// $Id: options_query_list.php,v 1.21 2018-07-12 14:08:52 dgoron Exp $
...
$options=(isset($options) ? stripslashes($options) : '');

if ($first==1) {
	$param["FOR"]="query_list";
	
	$param['QUERY'][0]['value']="<![CDATA[".str_replace("\r\n","\n",stripslashes($REQUETE))."]]>";
...
?>
<script>
opener.document.formulaire.<?php echo $name; ?>_options.value="<?php echo str_replace(array("\r", "\n"),array("\\r", "\\n"),addslashes($options)); ?>";
...
if (!$first) {
	...
	if (!isset($param["FOR"]) || $param["FOR"]!="query_list")  {
		$param=array();
		$param["FOR"]="query_list";
		...
		$param["INSERTAUTHORIZED"][0]['value'] = '';
		$param['UNSELECT_ITEM'][0]['VALUE'] = '';
		$param['UNSELECT_ITEM'][0]['value'] = '';
		$param['QUERY'][0]['value'] = '';
		$param["OPTIMIZE_QUERY"][0]['value'] = '';
		$param["FIELD0"][0]["value"] = '';
		$param["FIELD1"][0]["value"] = '';
	}
...
if ($first==2) {
	if (!$resultat) {
		echo "<span class='center'>".$msg['procs_options_echec_requete']."<br />".pmb_mysql_error()."</span>";
	} else {
		echo "<span class='center'><b>".$msg['procs_options_reponse_requete']."</b></span>";
