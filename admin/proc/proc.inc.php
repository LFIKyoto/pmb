...
function run_form($id, $dbh) {
	...
	else echo "<script>document.location='admin.php?categ=proc&sub=proc&action=final&id=".$id."&force_exec=".$force_exec."'</script>";
}
...
//			pmb_mysql_query($sql, $dbh) or die(pmb_mysql_error());
			...
			if (preg_match_all("|!!(.*)!!|U",$hp->proc->requete,$query_parameters)){
				$hp->gen_form("admin.php?categ=proc&sub=proc&action=final_remote&id=".$id."&force_exec=".$force_exec);
			}else{
				echo "<script>document.location='admin.php?categ=proc&sub=proc&action=final_remote&id=".$id."&force_exec=".$force_exec."'</script>";
			}
		}
		else echo "<script>document.location='admin.php?categ=proc&sub=proc&action=final_remote&id=".$id."&force_exec=".$force_exec."'</script>";
