...
// +-------------------------------------------------+
// $Id: sync_pnb.class.php,v 1.1.4.5 2019-12-03 08:59:55 ngantier Exp $
...
class sync_pnb extends scheduler_task {
	...
	/**
	 * suppression des prêts expirés
	 */
	public function clean_loans() {
	    
	    $query = "select pret_idexpl FROM pret WHERE pret_retour < CURDATE() AND pret_pnb_flag = 1";	    
	    $result = pmb_mysql_query($query);
	    if (pmb_mysql_num_rows($result)) {
	        while ($row = pmb_mysql_fetch_assoc($result)) {
	           pmb_mysql_query("delete from pnb_loans where pnb_loan_num_expl  ='" . $row['pret_idexpl'] . "' ");
	        }	        
    	    $query = "DELETE FROM pret WHERE pret_retour < CURDATE() AND pret_pnb_flag = 1";	    
	...
	public function import_onix2uni() {
		...
		global $pmb_pnb_codestat_id, $pmb_pnb_statut_id, $pmb_pnb_typedoc_id;
		...
		foreach ($xml as $content) {
			...
			foreach ($offers['offer'] as $offer) {
				...
				$data['typdoc'] = $pmb_pnb_typedoc_id;
				$data['cote'] = '-';
				$data['section'] = $deflt_docs_section;
				$data['statut'] = $pmb_pnb_statut_id;
				$data['location'] = $deflt_docs_location;
				$data['codestat'] = $pmb_pnb_codestat_id;
