...
if($form_edition_forcing) 	$droits = $droits + EDIT_FORCING_AUTH;
...
if($form_cms) $droits = $droits + CMS_AUTH;
...
if ($nbr > 0) {
	...
	if(!$id) {
		if(!empty($form_login) && $form_pwd==$form_pwd2) {
			...
			$requete.= "nb_per_page_gestion, user_email, user_alert_resamail, user_alert_demandesmail, user_alert_subscribemail, user_alert_suggmail, explr_invisible, explr_visible_mod, explr_visible_unmod";
			if (isset($sel_group)) {
				$requete.= ", grp_num";
			}
			$requete.= ") VALUES";
		...
		if($nbr==1) {
			...
			if ((!$acquisition_active) || (!$form_user_alert_suggmail)) $form_user_alert_suggmail="0" ;
