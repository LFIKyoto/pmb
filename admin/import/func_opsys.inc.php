...
// +-------------------------------------------------+
// $Id: func_opsys.inc.php,v 1.17 2019-08-01 13:16:34 btafforeau Exp $
...
function update_authors_num_opsys($aut_ ,$responsability_type)
{
	...
	for($i=0;$i<count($aut_);$i++ ) {
		if ($aut_[$i][3]!="") {
			$aut_i_4=htmlspecialchars($aut_[$i]['4'],ENT_QUOTES,$charset);
			$aut_i_p=htmlspecialchars($aut_[$i]['p'],ENT_QUOTES,$charset);
...
//		else $mots_cles = $index_sujets;
	...
	for ($a=0; $a<count($info_610_a); $a++) {
		$Libelle=$info_610_a[$a][0];
		for ($j=0; $j<count($info_610_e[$a]); $j++) {
			if($info_610_e[$a][$j]){
				$Libelle.=" : ".$info_610_e[$a][$j];
			}
		}
		//vérification de l'existence des categs
		$resultat = categories::searchLibelle(addslashes($Libelle), $num_thesaurus_610, 'fr_FR');
		if (!$resultat){
			// création
			$resultat = create_categ($thes_610,$thes_610->num_noeud_racine, $Libelle, ' '.strip_empty_words($Libelle).' ',$info_610_3[$a][0]);
		}
		// ajout de l'indexation à la notice dans la table notices_categories
		$rqt_ajout = "insert into notices_categories set notcateg_notice='".$notice_id."', num_noeud='".$resultat."' " ;
		$res_ajout = @pmb_mysql_query($rqt_ajout, $dbh);
...
/*
	for ($a=0; $a<count($info_610_a); $a++) {
		$resultat = categories::searchLibelle(addslashes($info_610_a[$a][0]), $num_thesaurus_610, 'fr_FR');
		if (!$resultat){
			//vérification de l'existence des categs, sinon création
