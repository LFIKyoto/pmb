...
// +-------------------------------------------------+
// $Id: func_livrjeun.inc.php,v 1.5 2015-04-03 11:16:23 jpermanne Exp $
...
function import_new_notice_suite() {
	...
	for ($a=0; $a<sizeof($info_601_a); $a++) {
		if ($info_601_a[$a][0]) $mots_cles[]=$info_601_a[$a][0] ;
		for ($j=0; $j<sizeof($info_601_j[$a]); $j++) if($info_601_j[$a][$j]) $mots_cles[]=$info_601_j[$a][$j] ;
		for ($j=0; $j<sizeof($info_601_x[$a]); $j++) if($info_601_x[$a][$j]) $mots_cles[]=$info_601_x[$a][$j] ;
		for ($j=0; $j<sizeof($info_601_y[$a]); $j++) if($info_601_y[$a][$j]) $mots_cles[]=$info_601_y[$a][$j] ;
		for ($j=0; $j<sizeof($info_601_z[$a]); $j++) if($info_601_z[$a][$j]) $mots_cles[]=$info_601_z[$a][$j] ;
		}
	for ($a=0; $a<sizeof($info_606_a); $a++) {
		if ($info_606_a[$a][0]) $mots_cles[]=$info_606_a[$a][0] ;
		for ($j=0; $j<sizeof($info_606_j[$a]); $j++) if($info_606_j[$a][$j]) $mots_cles[]=$info_606_j[$a][$j] ;
		for ($j=0; $j<sizeof($info_606_x[$a]); $j++) if($info_606_x[$a][$j]) $mots_cles[]=$info_606_x[$a][$j] ;
		for ($j=0; $j<sizeof($info_606_y[$a]); $j++) if($info_606_y[$a][$j]) $mots_cles[]=$info_606_y[$a][$j] ;
		for ($j=0; $j<sizeof($info_606_z[$a]); $j++) if($info_606_z[$a][$j]) $mots_cles[]=$info_606_z[$a][$j] ;
		}
	for ($a=0; $a<sizeof($info_607_a); $a++) {
		if ($info_607_a[$a][0]) $mots_cles[]=$info_607_a[$a][0] ;
		for ($j=0; $j<sizeof($info_607_j[$a]); $j++) if($info_607_j[$a][$j]) $mots_cles[]=$info_607_j[$a][$j] ;
		for ($j=0; $j<sizeof($info_607_x[$a]); $j++) if($info_607_x[$a][$j]) $mots_cles[]=$info_607_x[$a][$j] ;
		for ($j=0; $j<sizeof($info_607_y[$a]); $j++) if($info_607_y[$a][$j]) $mots_cles[]=$info_607_y[$a][$j] ;
		for ($j=0; $j<sizeof($info_607_z[$a]); $j++) if($info_607_z[$a][$j]) $mots_cles[]=$info_607_z[$a][$j] ;
		}
