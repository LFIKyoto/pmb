...
// +-------------------------------------------------+
// $Id: nav_collections.class.php,v 1.6 2017-10-27 13:39:50 ngantier Exp $

if (stristr($_SERVER['REQUEST_URI'], ".class.php")) die("no access");

global $class_path,$base_path,$include_path;

class nav_collections {    
	...
	public function get_form(){
		...
		require_once($base_path."/admin/opac/opac_view/filters/nav_collections/nav_collections.tpl.php");
		...
		for($i=0;$i<count($this->liste_item);$i++) {
			$liste_id[] = 'nav_collections_selected_'.$this->liste_item[$i]->id;
	...
	public function save_form(){
		...
		$selected_list=array();
		for($i=0;$i<count($this->liste_item);$i++) {
			eval("global \$nav_collections_selected_".$this->liste_item[$i]->id.";
			\$selected= \$nav_collections_selected_".$this->liste_item[$i]->id.";");
			if($selected){
