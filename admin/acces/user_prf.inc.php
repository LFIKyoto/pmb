...
foreach($t_cat as $k=>$v) {
	...
	if ($id==$k) {
		...
		$menu_sous_rub=$lib;
		$menu_sous_rub.= '&nbsp;&gt;&nbsp;'.htmlentities($dom->getComment('user_prf_lib'), ENT_QUOTES, $charset);
	}
...
function show_user_prf_list($id,$maj=false) {
	...
	$rows.= "<td>".htmlentities($dom->getComment('user_prf_def_lib'), ENT_QUOTES, $charset)."</td>";
	$rows.= "<td></td>";
	$rows.= "<td></td>";
	$rows.= "</tr>";
	...
	if (pmb_mysql_num_rows($r)) {
		...
		$selector.= "<option value=\"0\" >".htmlentities($dom->getComment('user_prf_def_lib'), ENT_QUOTES, $charset)."</option>";
	...
	$bt_calc = "<input type='button' onclick=\"
		this.form.action='./admin.php?categ=acces&sub=user_prf&action=calc&id=$id'; 
		this.form.submit();return false;\" 
		value=\"".$dom->getComment('user_prf_bt_calc')."\" class='bouton' />";
	$form = str_replace('<!-- bt_calc -->', $bt_calc,$form);

	$bt_enr = "<input type='button' onclick=\"
	this.form.action='./admin.php?categ=acces&sub=user_prf&action=update&id=$id'; 
	this.form.submit();return false;\" 
	value=\"".addslashes($msg['77'])."\" class='bouton' />";
	$form = str_replace('<!-- bt_enr -->', $bt_enr,$form);

	$bt_sup = "<input type='button' onclick=\"
		document.location='./admin.php?categ=acces&sub=user_prf&action=delete&id=$id';return false;\" 
...
function show_calc_user_prf_list($id) {
	...
	$bt_calc = "<input type='button' onclick=\"
		this.form.action='./admin.php?categ=acces&sub=user_prf&action=calc&id=$id'; 
		this.form.submit();return false;\" 
		value=\"".addslashes($dom->getComment('user_prf_bt_calc'))."\" class='bouton' />";
	$form = str_replace('<!-- bt_calc -->', $bt_calc,$form);
