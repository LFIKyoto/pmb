...
// +-------------------------------------------------+
// $Id: pmb.class.php,v 1.7.4.1 2015-09-15 14:32:56 apetithomme Exp $
...
class xml_dom_pmb {
	 ...
	 * \verbatim
	 ...
	 a/b/id@c[3]	Le troisième noeud élément c (<c id="2">Texte 2</c>). L'attribut est ignoré
	 ...
	 * @param string $path chemin des noeuds recherchés
	 * @param noeud $node Noeud de départ de la recherche
	 * @return array Tableau des données sérialisées ou des valeurs de l'attribut, \b false si le chemin n'existe pas
	 * \note Exemples de valeurs renvoyées selon le chemin :
	 * \verbatim
	 <a>
	 	<b>
...
//Fonction de recherche
	...
	function rec_records($notice_xml_uni, $source_id, $search_id) {
		...
		if ($notice_xml_uni && $this->current_protocole) {
			...
			if (($add_explnum) && is_array($notice_xml_uni["noticeExplNums"]) && count($notice_xml_uni["noticeExplNums"]) ) {
