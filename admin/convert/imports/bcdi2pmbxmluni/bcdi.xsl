...
<xsl:template match="NOTICES">
	<notice>
		...
		<!-- Périodiques -->
		<xsl:choose>
			<xsl:when test="./DOCUMENTS/SUPPORT_D='Périodique'">
				<xsl:call-template name="periodiques"/>
			</xsl:when>
			<xsl:when test="(./TYPE_NOTICE_N='Partie') and  (./DOCUMENTS/IDENTITE_D_D != IDENTITE_N_N)">
...
<xsl:template name="langue">
	<xsl:if test="LANGUE_N">
		<xsl:call-template name="construct_repeat">
			<xsl:with-param name="chaine" select="translate(LANGUE_N,&MAJUS_EN_MINUS;)"/>
...
<xsl:template name="date_creation">
	<xsl:if test="DATE_SAISIE_N_N">
		<xsl:element name="f">
			<xsl:attribute name="c">100</xsl:attribute>
			<xsl:element name="s">
				<xsl:attribute name="c">a</xsl:attribute>
				<xsl:value-of select="substring-after(substring-after(DATE_SAISIE_N_N,'/'),'/')"/>
				<xsl:value-of select="substring-before(substring-after(DATE_SAISIE_N_N,'/'),'/')"/>
				<xsl:value-of select="substring-before(DATE_SAISIE_N_N,'/')"/>
			</xsl:element>	
		</xsl:element>
	</xsl:if>
</xsl:template>
...
<xsl:template name="mots_clefs" match="MOTS_CLES_N">
	<xsl:if test="MOTS_CLES_N">
		<xsl:call-template name="construct_repeat">
			<xsl:with-param name="chaine" select="MOTS_CLES_N"/>
...
<xsl:template name="persos">
	...
	<!-- Date de saisie -->
	<xsl:if test="DOCUMENTS/DATE_DE_SAISIE_D">
		<xsl:element name="f">
			<xsl:attribute name="c">904</xsl:attribute>
			<xsl:element name="s">
				<xsl:attribute name="c">a</xsl:attribute>
				<xsl:value-of select="substring-after(substring-after(DOCUMENTS/DATE_DE_SAISIE_D,'/'),'/')"/>
				<xsl:text>-</xsl:text>
				<xsl:value-of select="substring-before(substring-after(DOCUMENTS/DATE_DE_SAISIE_D,'/'),'/')"/>
				<xsl:text>-</xsl:text>
				<xsl:value-of select="substring-before(DOCUMENTS/DATE_DE_SAISIE_D,'/')"/>
			</xsl:element>			
		</xsl:element>
	</xsl:if>
...
<xsl:template name="url">
	<xsl:choose>
		<xsl:when test="RESSOURCES/ADRESSE_L">
			<xsl:element name="f">
		...
		<xsl:when test="RESSOURCES/RESSOURCE_L">
			<xsl:element name="f">
				<xsl:attribute name="c">856</xsl:attribute>
				<xsl:element name="s">
					<xsl:attribute name="c">u</xsl:attribute>
					<xsl:value-of select="RESSOURCES/RESSOURCE_L"/>
				</xsl:element>			
			</xsl:element>
		</xsl:when>
	</xsl:choose>
...
<xsl:template name="periodiques">
	<xsl:element name="f">
		...
		<xsl:if test="DOCUMENTS/DOCUMENT_D">
			<xsl:element name="s">
				<xsl:attribute name="c">o</xsl:attribute>
				<xsl:value-of select="DOCUMENTS/DOCUMENT_D"/>
			</xsl:element>
		</xsl:if>
		...
		<xsl:if test="DOCUMENTS/ISSN_D">
			<xsl:element name="s">
				<xsl:attribute name="c">x</xsl:attribute>
				<xsl:value-of select="DOCUMENTS/ISSN_D"/>
			</xsl:element>
		</xsl:if>
	</xsl:element>
</xsl:template>
...
<xsl:template name="chapitre">
	<xsl:if test="normalize-space(DOCUMENTS/DOCUMENT_D) != ''">
		<xsl:element name="f">
		<xsl:attribute name="c">463</xsl:attribute>
			...
			<xsl:element name="s">
				...
				<xsl:value-of select="concat('id:',DOCUMENTS/IDENTITE_D_D)"/>
			</xsl:element>
			<xsl:if test="normalize-space(DOCUMENTS/ISBN_D) != ''">
				<xsl:element name="s">
					<xsl:attribute name="c">y</xsl:attribute>
					<xsl:value-of select="DOCUMENTS/ISBN_D"/>
				</xsl:element>
			</xsl:if>
			<s c='9'>bl:m0</s>
			<s c='9'>type_lnk:d</s>
	...
	<xsl:if test="EXEMPLAIRES[$n_ex]">	
		<xsl:element name="f">
			...
			<xsl:if test="TYPES_NATURE_N">
				<xsl:element name="s">
					<xsl:attribute name="c">t</xsl:attribute>
					<xsl:value-of select="normalize-space(substring-before(TYPES_NATURE_N,'/'))"/>
				</xsl:element>
			</xsl:if>	
			<!-- Commentaire -->
